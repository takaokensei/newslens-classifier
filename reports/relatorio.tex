...
\midrule
\textbf{Total} & \textbf{315} & \textbf{100\%} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption{Eficiência e Performance Global dos Modelos}
...
\midrule
TF-IDF + SVM & 0.968 & 0.968 & 0.140 & 0.079 & 0.182 \\
TF-IDF + XGBoost & 0.704 & 0.714 & 0.420 & 0.107 & 0.489 \\
BERT + SVM & 1.000 & 1.000 & 0.120 & 2.228 & 0.875 \\
BERT + XGBoost & 0.967 & 0.968 & 0.377 & 2.296 & 0.428 \\
\bottomrule
\end{tabular}%
}
\end{table}
...
\begin{table}[H]
\centering
\caption{F1-Score por Classe e Modelo}
\label{tab:classes}
\begin{tabular}{lcccc}
\toprule
...
% Placeholder para gráficos F1 por classe, Accuracy comparativa, Latência vs Performance
% Estes podem ser gerados usando os dados das tabelas
...
A análise identificou 2 casos onde o BERT classificou corretamente enquanto o TF-IDF falhou. As explicações geradas pelo LLM destacaram:

\begin{itemize}
    \item \textbf{Contexto semântico}: O BERT captura relações entre palavras que vão além da frequência léxica
    \item \textbf{Ambiguidade lexical}: Palavras que aparecem em múltiplos contextos são melhor interpretadas pelo BERT
    \item \textbf{Estrutura sintática}: A ordem e estrutura das frases influenciam a classificação semântica
\end{itemize}

Os resultados completos estão disponíveis em \texttt{models/differential\_errors.json}.
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item Geração de explicações via LLM (opcional)
        \item Salvamento automático em log
    \end{itemize}
    \item \textbf{Monitoramento}: Dashboard com estatísticas
    \begin{itemize}
        \item Métricas agregadas (total de predições, score médio)
        \item Gráficos de distribuição por classe e modelo
        \item Evolução temporal das predições
        \item Tabela de predições recentes
    \end{itemize}
\end{enumerate}

O sistema suporta seleção de idioma (Português/English) com português como padrão.
...
\begin{itemize}
    \item Lê arquivos .txt ou .csv da pasta
    \item Classifica cada texto usando o modelo selecionado
    \item Registra todas as predições no log
    \item Gera um relatório resumido
\end{itemize}
...
\textbf{Vantagens:}
\begin{itemize}
    \item Compreensão semântica profunda
    \item Performance superior (F1=1.0)
    \item Captura relações contextuais entre palavras
    \item Robustez a variações lexicais
\end{itemize}

\textbf{Desvantagens:}
\begin{itemize}
    \item Cold start significativo (2.23s)
    \item Maior uso de memória
    \item Menor interpretabilidade
    \item Dependência de GPU para treinamento (não para inferência)
\end{itemize}
