...
\textbf{Observações:}
\begin{itemize}
    \item BERT + SVM alcançou performance perfeita (F1=1.0, Accuracy=1.0) no conjunto de teste
    \item TF-IDF + SVM oferece excelente eficiência (0.14ms/doc) com performance competitiva (F1=0.968)
    \item Cold start do BERT é 28x maior que TF-IDF (2.23s vs 0.08s)
    \item TF-IDF + XGBoost apresentou performance inferior (F1=0.704)
\end{itemize}
...
\begin{table}[H]
\centering
\caption{F1-Score por Classe e Modelo (Conjunto de Teste)}
\label{tab:classes}
\begin{tabular}{lcccc}
\toprule
